...
import { collectCost, SHOW_BASE_CPU_COST } from './cpuLogger';
...
export const createApp = function(opt: Partial<CreateOptions> = {}) {
	...
	let _memoryCacher: MemoryCacher = (next) => {
		if (lastTime && _cachedMemory && Game.time == lastTime + 1) {
			// @ts-ignore
	...
	const runAllRoom = () => Object.values(Game.rooms).map((room) => errorMapper(roomRunner as any, room));
	const runAllCreep = () => Object.values(Game.creeps).map((creep) => errorMapper(creepRunner as any, creep));
	const runAllPowerCreep = () => Object.values(Game.powerCreeps).map((creep) => errorMapper(powerCreepRunner as any, creep));
	...
	const _run = function(): void {
		...
		let cpu_test = false;
		if (Memory.Systemswitch && Memory.Systemswitch.Showtestrun) {
			cpu_test = true;
		}
